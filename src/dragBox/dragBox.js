...
import "./dragBox.css";
import { transform, formatter } from "./dictFormat";
...
export class DragBox extends PureComponent {
  ...
  handleTrack = (dir) => {
    ...
    */
    ...
    const align = this.state.notFound && this.state.tracker.length > 1 ? -1 : 0;
    ...
    this.setState(
      ...
      () => {
        //if tracker array isnt empty and notfound and also 0, remain 0
        this.handleSearch(
          this.container,
...
*/
    ...
    */
    ...
    this.setState({ searching: true }, () => {
      ...
      this.setState({ text: value }, () => {
        ...
        axios
          .post("http://localhost:6060/api/dictSearch", JSON.stringify(body), {
            headers: {
  ...
  expand = () => {
    ...
    */
    ...
    let finalVal;
    if (diff <= 36 && this.container.getBoundingClientRect().height === 160) {
      return;
    } else {
          ...
          <div className="dragBox_bottom_left">
            <i
              disabled={
                ...
                (this.state.notFound && this.state.tracker.length === 0)
              }
              onClick={() => {
                if (
                  (!this.state.notFound &&
        ...
        >
          ...
          {this.state.result.length && !this.state.searching ? (
            this.state.result.map((data, n) => (
              <div key={`formatter_key_sense_${n}`} className="sense_sect">
                {data.map((dtxt, m) => (
                        ...
                        typeof txt === `object` ? (
                          txt.format === `link` ? (
                            ...
                            */
                              ...
                              <b
                                ...
                                onClick={() =>
                                  this.handleSearch(this.container, txt.text)
                                }
                          ...
                          */
                              ...
                              <span
                                style={{
                                  fontStyle: "italic",
                                  fontWeight: txt.both > 0 ? `bold` : `inherit`,
                                }}
                                dangerouslySetInnerHTML={{
                                  __html: txt.text,
                                }}
            ...
            <div key={`formatter_key_empty`} className="drag_box_empty center">
